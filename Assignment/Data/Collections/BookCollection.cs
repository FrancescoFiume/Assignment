...
public class BookCollection : IObjectCollection<BookDbo>
{
    ...
    /// </summary>
    /// <param name="bookDbo"> Books that needs to be added</param>
    /// <returns>gives back the item created with id and registration date</returns>
    public BookDbo Add(BookDbo bookDbo)
    {
        var newBook = new BookDbo()
        {
            //No Id, it gets generated by postgre serial
            Title = bookDbo.Title,
            Author = bookDbo.Author,
            ISBN = bookDbo.ISBN,
    ...
    /// </summary>
    /// <param name="bookDbo"></param>
    public void Update(BookDbo bookDbo)
    {
        using var scope = _serviceProvider.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<AppDbContext>();
        ...
        var bookToUpdateDb = context.Books.First(b => b.Id == bookDbo.Id);
        Type userType = bookDbo.GetType();
        PropertyInfo[] properties = userType.GetProperties();
        foreach (var property in properties)
        {
            if (property.PropertyType == typeof(string) && property.GetValue(bookDbo) as string != "")
            {

                var toUpdateProperty = typeof(BookDbo).GetProperty(property.Name);
                toUpdateProperty!.SetValue(bookToUpdateDb, property.GetValue(bookDbo));
            }
        ...
        if (IsCacheUsed)
        {
            int index = _cache.FindIndex(b => b.Id == bookDbo.Id);
    ...
    public void ToggleAvailability(int id)
    {
        ...
        if (toUpdate.Status == Availability.Available)
        {
            toUpdate.Status = Availability.Unavailable;
        }
        else
        {
            toUpdate.Status = Availability.Available;
        }
    ...
    /// </returns>
    public IEnumerator<BookDbo> GetEnumerator()
    {
        if (IsCacheUsed)
        {
